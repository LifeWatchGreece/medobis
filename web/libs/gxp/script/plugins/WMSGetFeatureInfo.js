...
gxp.plugins.WMSGetFeatureInfo = Ext.extend(gxp.plugins.Tool, {
    ...
    addActions: function() {
        ...
        var updateInfo = function() {
            // Checks for each layer in app.mapPanel.layers has the atribute "queryable" set and its value is true
            ...
            info.controls = [];
            // for each querable layer 
            queryableLayers.each(function(x){               
                var layer = x.getLayer(); // this return the relevant layer object from app.mapPanel.map.layers
                    ...
                    eventListeners: {
                        getfeatureinfo: function(evt) {
                            ...
                            if (infoFormat == "text/html") {
                                ...
                                //alert("0-"+match[1]); match[1] is the part between the body opening and closing tags
                                ...
                                //alert("000-"+x.get("getFeatureInfo"));
                    ...
                    eventListeners: {
                        getfeatureinfo: function(evt) {
                            ...
                            if (evt.features && evt.features.length > 0) {
                                this.displayPopup2(evt, title, null,  x.get("getFeatureInfo"));
                            }
                        },
                        scope: this
                    }
    ...
    displayPopup2: function(evt, title, text, featureinfo) {
        var popup;
        var popupKey = evt.xy.x + "." + evt.xy.y;
        featureinfo = featureinfo || {};
        if (!(popupKey in this.popupCache)) {
            popup = this.addOutput({
                xtype: "gx_popup",  // GeoExt.Popup
                // Properties inhereted from Ext.Window
                ...
                autoScroll: true,
                fill: false, // ?
                // Properties that belong to GeoExt.Popup
                location: evt.xy,
        ...
        if (!text && features) {
            var feature;
            var featureHtml = "<table class='table'>";
            this.target.mapPanel.myfeatures = [];
            // Iterating through the list of features (OpenLayers.Feature.Vector)
            for (var i=0,ii=features.length; i<ii; ++i) {
                feature = features[i];
                var attributes = feature.attributes;
                if(i==0){
                    featureHtml = featureHtml + "<thead>";
                     for (var key in attributes) {
                        if (attributes.hasOwnProperty(key)) {  
                            featureHtml = featureHtml + "<th>" + key + "</th>";
                        }
                    }                    
                    featureHtml = featureHtml+"</thead><tbody><tr>";
                } else {
                    featureHtml = featureHtml + "<tr>";
                }                                
                //this.target.mapPanel.myfeatures[i] = feature;    
                for (var key in attributes) {
                    if (attributes.hasOwnProperty(key)) {      
                        var aValue = attributes[key];
                        featureHtml = featureHtml + "<td>" + aValue + "</td>";
                    }
                }
                if(i==(ii-1)){
                    featureHtml = featureHtml+"</tr></tbody>";
                } else {
                    featureHtml = featureHtml + "</tr>";
                }   
            }
    ...
    displayPopup: function(evt, title, text, featureinfo) {
        ...
        var features = evt.features, config = [];
        if (!text && features) { // if list of features were sent
            alert('1');
            var feature;
            for (var i=0,ii=features.length; i<ii; ++i) {
                feature = features[i];
        ...
        popup.add(config); // add extra configuration to popup
        popup.doLayout();
    }
