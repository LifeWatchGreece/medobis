...
/** api: constructor
 *  .. class:: AddViewerLayers(config)
 *
 *    Plugin for removing a selected layer from the map.
 *    TODO Make this plural - selected layers
 */
gxp.plugins.AddViewerLayers = Ext.extend(gxp.plugins.Tool, {
    
    /** api: ptype = gxp_addviewerlayers */
    ptype: "gxp_addviewerlayers",
    ...
    constructor: function(config) {
        this.addEvents(
             ...
             *  Listener arguments:
             *
             *  * tool - :class:`gxp.plugins.AddViewerLayers` This tool.
             *  * source - :class:`gxp.plugins.LayerSource` The selected source.
             */
            "sourceselected"
        );
        gxp.plugins.AddViewerLayers.superclass.constructor.apply(this, arguments);        
    },
    ...
    addActions: function() {
        ...
        var actions = gxp.plugins.AddViewerLayers.superclass.addActions.apply(this, [options]);
    ...
    showCatalogueSearch: function() {
        ...
        var output = gxp.plugins.AddViewerLayers.superclass.addOutput.apply(this, [{
            sources: sources,
    ...
    initCapGrid: function() {
        ...
        function addViewerLayers() {
            ...
            function collectRecords(record) {
                ...
                numRecords--;
                if (numRecords === 0) {
                    this.addViewerLayers(recordsToAdd);
                }
            }
    ...
    addViewerLayers: function(records, isUpload) {
        ...
        for (var i=0, ii=records.length; i<ii; ++i) {
            ...
            // record is an instance of GeoExt.data.LayerRecord class
            record = source.createLayerRecord({
                name: records[i].get("name"),
                source: source.id
            }) || records[i];
            if (record) {                
                layer = record.getLayer(); // layer is an instance of OpenLayers.Layer class
                if (layer.maxExtent) {
                    if (!extent) {
                ...
                this.fireEvent("wmslayeradded", this, record.get('name'));
                this.target.tools['savewmslayertool'].fireEvent("globallayeradded",record.get('name'),layer);
            }
        }
        if (extent) {
                                    ...
                                    this.selectedSource.store.load({
                                        ...
                                        callback: function(records, options, success) {
                                            ...
                                            if (gridPanel) {
                                                ...
                                                this.addViewerLayers(newRecords, true);
                                            }
                                        },
                                        scope: this
                                    });
...
Ext.preg(gxp.plugins.AddViewerLayers.prototype.ptype, gxp.plugins.AddViewerLayers);
