    ...
    addOutput: function(config) {
        ...
        config = Ext.apply({
            ...
            bbar: (featureManager.paging ? [{
                ...
                disabled: true,
                hidden: (featureManager.pagingType !== gxp.plugins.FeatureManager.QUADTREE_PAGING) || featureManager.autoZoomPage,
                handler: function() {
                ...
                handler: function() {                    
                    ...
                    var delimiter = ";";
                    var keyString = "";
                    var keysN = featureManager.featureStore.fields.length;       // get the number of columns/keys
                    var recordsN = featureManager.featureStore.getCount();       // get the number of rows
                    
                    //alert('Number of keys = '+keysN);
                    //alert('Number of records = '+recordsN);
                    
                    emptyString = true;
                    featureManager.featureStore.fields.eachKey(function(keyValue){
                        
                        if((keyValue == 'feature')||(keyValue == 'state')||(keyValue == 'fid')){
                            // do nothing. These are not grid columns
                        } else {
                            if(emptyString)
                                keyString = keyValue;
                            else
                                keyString = keyString+delimiter+keyValue;
                            emptyString = false;
                        }                    
                        
                    });
                    
                    dataArray.push(keyString);  // The first row contains the column names
                    //alert('keyString = '+keyString);
                    var keys = keyString.split(delimiter);
                                        
                    // Loop througth the features of the grid                    
                    featureManager.featureStore.data.each(function(record){      
                        //alert('record index = '+recordIndex);
                        var rowString = "";
                        emptyString = true;
                        for(var j = 0; j < keys.length; j++){
                            
                            if((keys[j] == 'feature')||(keys[j] == 'state')||(keys[j] == 'fid')){
                                // do nothing. This is an object of type e.g OpenLayers.Feature.Vector
                            } else {
                                if(emptyString){
                                    rowString = record.data[keys[j]];
                                } else {
                                    rowString = rowString+delimiter+record.data[keys[j]];
                                }
                                emptyString = false;
                            }                                                        
                        }
                    ...
                    var jsonString = Ext.util.JSON.encode(dataArray);
                    
                    Ext.Ajax.request({
                        url: this.baseUrl+'/saveGridFeatures',                        
                        method: "POST",
                        jsonData: jsonString,
                        ...
                        success: function(response) {
                            ...
                            Ext.Msg.show({
                                ...
                                msg: "<div><p style='margin: 10px'>You can download the data using the following links. The links will be available for at least 5 minutes. </p> <p style='margin:10px'> CSV: <a href='"+jsonData.csv+"'>Download</a><br>KML: <a href='"+jsonData.kml+"'>Download</a></p></div>",
                                buttons: Ext.Msg.OK,
                                width: 400
                            });                                                    
                        },
